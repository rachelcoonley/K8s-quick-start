...
From the **Kubernetes** menu, select one of the following options:
   * **Dashboard** &mdash; Access the native Kubernetes dashboard in a new browser window by clicking **Kubernetes UI**.
   * **CLI** &mdash; On the Kubernetes CLI page, two options display. To configure `kubectl` for your local machine, click **Generate Config**. Copy and paste the code that displays into your `.kube/config` file, and then run `kubectl`. You can also conveniently access the shell in a managed `kubectl` instance right in your web browser.
